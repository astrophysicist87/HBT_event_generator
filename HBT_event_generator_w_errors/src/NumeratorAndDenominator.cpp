...
//=====================================================================================
//=====================================================================================
void HBT_event_generator::Compute_numerator_and_denominator_with_errors_mode2(
							...
							)
{
	...
	#pragma omp parallel for schedule(static) shared( in_numerator, in_numerator2,\
														...
														in_qo_mean_diff, in_qs_mean_diff,\
														in_ql_mean_diff, in_diff_numPair_count )
	for (int iEvent = 0; iEvent < allEvents.size(); ++iEvent)
	{
		EventRecord event = allEvents[iEvent];
		...
		vector<double> private_qo_mean_diff(in_qo_mean_diff.size(), 0.0);
		vector<double> private_qs_mean_diff(in_qs_mean_diff.size(), 0.0);
		vector<double> private_ql_mean_diff(in_ql_mean_diff.size(), 0.0);
		vector<double> private_diff_numPair_count(in_diff_numPair_count.size(), 0.0);
		...
		for (int jParticle = 0; jParticle < event.particles.size(); ++jParticle)
		{
			...
			private_diff_numPair_count[index6D]++;
			private_numPair[index3D]++;

			// allows slight adjustment to bin location
			private_qo_mean_diff[index6D] += qo;
			private_qs_mean_diff[index6D] += qs;
			private_ql_mean_diff[index6D] += ql;
		}
		...
		#pragma omp critical
		{
			...
			for (int iKL = 0; iKL < n_KL_bins; iKL++)
			{
				...
				for (int iql = 0; iql < n_ql_bins; iql++)
				{
					...
					// combine q bin location averages
					in_qo_mean_diff[idx6D] += private_qo_mean_diff[idx6D];
					in_qs_mean_diff[idx6D] += private_qs_mean_diff[idx6D];
					in_ql_mean_diff[idx6D] += private_ql_mean_diff[idx6D];
					in_diff_numPair_count[idx6D] += private_diff_numPair_count[idx6D];
